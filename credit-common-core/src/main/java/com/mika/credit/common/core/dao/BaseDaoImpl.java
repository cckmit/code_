package com.mika.credit.common.core.dao;
...
import com.mika.credit.common.exceptions.ServiceException;
...
public abstract class BaseDaoImpl<T extends BaseEntity> extends SqlSessionDaoSupport implements BaseDao<T> {
	...
	public int deleteById(int id) {
		return getSqlSession().delete(getStatement(SQL_DELETE_BY_ID), id);
	}
	...
	public PageBean listPage(PageParam pageParam, Map<String, Object> paramMap, String sqlId) {
		...
		// 获取分页数据集 , 注切勿换成 getSqlSession() 对象
	...
	public PageBean listPage(PageParam pageParam, Map<String, Object> paramMap) {
		...
		if (isCount != null && "1".equals(isCount.toString())) {
			Map<String, Object> countResultMap = getSqlSession().selectOne(getStatement(SQL_COUNT_BY_PAGE_PARAM), paramMap);
	...
	public List<Object> listBy(Map<String, Object> paramMap, String sqlId) {
		...
		return getSqlSession().selectList(getStatement(sqlId), paramMap);
	}
	...
	public String getSeqNextValue(String seqName) {
		...
		try {
			...
			throw ServiceException.DB_GET_SEQ_NEXT_VALUE_ERROR.newInstance("获取序列出现错误!序列名称:{%s}", seqName);
		} finally {
			if (isClosedConn) {
