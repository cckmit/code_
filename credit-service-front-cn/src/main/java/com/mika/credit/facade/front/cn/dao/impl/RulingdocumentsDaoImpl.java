package com.mika.credit.facade.front.cn.dao.impl;

import com.mika.credit.common.core.dao.BaseDaoImpl;
import com.mika.credit.common.entity.report.cn.Rulingdocuments;
import com.mika.credit.facade.front.cn.dao.RulingdocumentsDao;
import org.springframework.stereotype.Repository;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

/**
 * Created by dell on 2017/2/27.
 */
@Repository("rulingdocumentsDao")
public class RulingdocumentsDaoImpl extends BaseDaoImpl<Rulingdocuments> implements RulingdocumentsDao {
    @Override
    public Rulingdocuments findById(Integer id) {
        Rulingdocuments result = getById(id);
        return result;
    }

    @Override
    public List<Rulingdocuments> findByFullReportId(Integer fullReportId) {
        Map<String,Object> params = new HashMap<>();
        params.put("fullReportId",fullReportId);
        List<Rulingdocuments> result = listBy(params);
        return result;
    }
}
