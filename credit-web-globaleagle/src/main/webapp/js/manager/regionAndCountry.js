...
$(document).ready(function () {
    $("#continentForm").bootstrapValidator('validate');
...
function loadContinents(){
    $.post(basePath + 'manager/continentList.do',{},function (data) {
...
function continentDel(id) {
    ...
    BootstrapDialog.confirm("确定删除吗？",function(result) {
        if (result) {
            $.post(basePath + 'manager/continentDel.do',
                {
                    id: id,
                },
                function (data) {
...
function editCountry(id){
    ...
    if(id > 0){//加载国家信息
        $.post(basePath + 'manager/getCountry.do',{id:id},function(data){
            var map = $.parseJSON(data);
...
function countrySave() {
    if(!$("#countryForm").data('bootstrapValidator').validate().isValid())
        return;
    $.post(basePath + 'manager/countrySave.do',
        {
            id: $("#idCountry").val(),
...
function countryDel(id) {
    ...
    BootstrapDialog.confirm("确定删除吗？",function(result) {
        if (result) {
            $.post(basePath + 'manager/countryDel.do',
                {
                    id: id,
                },
                function (data) {
